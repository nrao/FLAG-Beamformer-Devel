    ...
    for (z=0; z<10; ++z)
    {
        for (i=0; i<nphases; ++i)
        {
            for (j=0; j<10; ++j)
            {
                counts += 1000;
                // printf("i=%d,j=%d,z=%d\n", i,j,z);
                exp_rdy = new_input_state(p, p->accumid_table[i], 0);
                if (exp_rdy)
                    printf("exp_ready, i=%d, j=%d, z=%d\n", i,j,z);
            }
        }
    }
    if(new_input_state(p, p->accumid_table[0], 0))
...
int test_exposures_by_counts()
{
    ...
    SwitchingStateMachine *p = create_switching_state_machine(nphases, sr, cal, ncycles, counts_per_exp);
    if (!p)
    {
        printf("create failed\n");
        return 0;
