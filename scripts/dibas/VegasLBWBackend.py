...
class VegasLBWBackend(Backend):
    ...
    * *unit_test:* Set to true to unit test. Will not attempt to talk to
    ...
    def __init__(self, theBank, theMode, theRoach, theValon, hpc_macs, unit_test = False):
        ...
        self.params["lbw_gain"     ] = self.setLBWGain
        ...
        # In LBW mode the spec_tick is computed differently than in HBW mode
    ...
    def prepare(self):
        ...
        if self.roach:
            ...
            self.roach.write('gain', self._gain)
    ...
    def _sampler_frequency_dep(self):
        ...
        if mode > 3 and mode < 10:
            self.sampler_frequency = self.frequency * 1e6 / 4
            self.nsubband = 1
        elif mode > 9 and mode < 20:
            self.sampler_frequency = self.frequency * 1e6 / 32
            self.nsubband = 1
        elif mode > 19 and mode < 30:
            self.sampler_frequency = self.frequency * 1e6 / 32
            self.nsubband = 8
        else:
            throw(Exception("Mode %i is not supported." % mode))
    ...
    def _set_state_table_keywords(self):
        ...
        # TBF: This is OK for nsubband of 1. There is an nsubband case of 8
